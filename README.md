<h1 align="center">Hi, I'm Arbaz </h1>
<h3 align="center">AI/ML Engineer • Researcher • Open Source Contributor</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=arbaz-hai&label=Profile%20Views&color=brightgreen&style=flat" alt="profile-views" />
</p>

<p align="center">
  <b>B.Tech CSE @ GGSIPU | AI/ML Graduate Trainee @ VALAY | Innovation Ambassador (MoE)</b>
</p>

---

## 🚀 About Me

- 🎓 B.Tech Computer Science student at **GGSIPU** with a **CGPA of 9.03/10**
- 🤖 Working as an **AI/ML Graduate Trainee** at **VALAY**
- 📄 Author of **research papers (Published)**
- 🔬 Interested in **Computer Vision, Semantic Segmentation, LLMs, and RAG systems**
- 🏆 Winner — **IIT Delhi Hackathon 2025**
- 💡 **Innovation Ambassador** at the **Ministry of Education’s Innovation Cell**
- 🌍 Founder of **Dev Hive Community** with **2000+ members across 5+ countries**
- 👨‍💻 Solved **550+ LeetCode problems** and ranked in the **Top 12%**
- 🤝 Open to collaborating on **AI, ML, NLP, Computer Vision, and LLM applications**

📫 **Email:** arbazsince2004@gmail.com

---

## Connect With Me

<p align="left">
  <a href="https://www.linkedin.com/in/arbaz-sheikh-712408339" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" height="30" width="40" />
  </a>
  <a href="https://github.com/Arbaz-hai" target="_blank">
    <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" height="30" width="40" />
  </a>
</p>

---

## 🧠 Tech Stack

### Languages
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/cplusplus/cplusplus-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/sqlite/sqlite-original.svg" width="40" height="40"/>
</p>

### Machine Learning / Deep Learning
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pytorch/pytorch-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/tensorflow/tensorflow-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/numpy/numpy-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pandas/pandas-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/opencv/opencv-original.svg" width="40" height="40"/>
</p>

### LLM / NLP / Tools
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/fastapi/fastapi-original.svg" width="40" height="40"/>
</p>

---

## 🔥 Featured Projects

### 🏜️ Off-Road Semantic Segmentation
**Tech:** Python, PyTorch, ResNet34  
- Built an encoder–decoder model for off-road terrain segmentation
- Achieved **89.43% IoU**
- Used data augmentation and GPU-optimized training for better generalization

### 🚀 Space Station Safety Object Detection
**Tech:** Python, PyTorch, YOLOv8, OpenCV  
- Developed a real-time safety object detection pipeline
- Achieved **95% accuracy** on held-out test data
- Detected critical objects under varying lighting and noise conditions

### 🌐 AI-Powered Human Trafficking Route Mapping Platform
**Tech:** NLP, SpaCy, Transformers, Network Analysis  
- Built an NLP pipeline to analyze dark web ads and phone records
- Used NER and relation extraction to map trafficking routes
- Created a domain-specific annotated dataset for fine-tuning transformer models

###  Government Fraud Detection System
**Tech:** Python, Scikit-Learn, Machine Learning  
- Implemented Random Forest and Isolation Forest for anomaly detection
- Identified ghost beneficiaries using anomaly scoring
- Achieved **95.6% accuracy**

### 👗 ANN Fashion-MNIST Classification
**Tech:** Python, PyTorch, ANN  
- Built a multi-layer ANN with ReLU, dropout, and Adam optimizer
- Achieved **92.8% test accuracy**
- Analyzed confusion matrix for class-wise errors

---

## 📄 Research

- **Accuracy–Energy Trade-off Analysis of Lightweight Semantic Segmentation Models for Desert Off-Road Navigation**
- **A Comprehensive Review of Graph Convolutional Networks for Node Classification: Methods, Challenges, and Future Directions**

---

## 🏆 Achievements

- 🥇 Winner — IIT Delhi Hackathon 2025
- 💡 Innovation Ambassador — Ministry of Education’s Innovation Cell
- 📄 2 Research Papers Accepted / In Press
- 👨‍💻 LeetCode Top 12%
- 🏅 550+ LeetCode problems solved
- 🎤 3× Speaker
- 👨‍🏫 3× Hackathon Mentor
- 🏁 10× Hackathon Finalist
- 🌍 Founder of a 2000+ member tech community

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=arbaz-hai&show_icons=true&theme=default" alt="GitHub Stats" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=arbaz-hai&theme=default" alt="GitHub Streak" />
</p>

---

## ✨ Fun Fact

I enjoy solving ML problems, building research-driven projects, and growing tech communities.

---

### 💙 If you like my work, consider giving my repositories a star!
